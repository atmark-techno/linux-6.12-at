...
static int spi_rpmsg_transfer_one(struct spi_controller *ctlr,
				  struct spi_device *spi,
				  struct spi_transfer *transfer)
{
	struct rpmsg_spi *devdata = spi_controller_get_devdata(ctlr);
	struct spi_rpmsg_msg rmsg;
	int status = 0;

	if (transfer->len > SPI_RPMSG_MAX_BUF_SIZE) {
		dev_warn(&ctlr->dev, "Failing transfer of %d bytes (%ld max)\n",
...
static int spi_rpmsg_platform_probe(struct platform_device *pdev)
{
	...
	ctlr = devm_spi_alloc_host(dev, sizeof(*devdata));
	if (!ctlr)
		return -ENOMEM;
	...
	ctlr->transfer_one = spi_rpmsg_transfer_one;
...
static void spi_rpmsg_platform_remove(struct platform_device *pdev)
{
