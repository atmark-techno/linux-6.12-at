...
static void ox01f10_reset_gpio(struct gpio_desc *gpio, int value);
			   ...
			   struct v4l2_subdev_format *format)
{
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY) {
		*v4l2_subdev_state_get_format(state, format->pad) = *mf;
		return 0;
	}
...
static struct ox01f10_power_rail *ox01f10_parse_dt(struct i2c_client *client)
{
	...
	gpio = devm_gpiod_get(&client->dev, "pwdn", GPIOD_OUT_LOW);
	if (IS_ERR(gpio)) {
		dev_err(&client->dev, "pwdn gpios not in DT\n");
		goto error;
	}

	power->pwdn_gpio = gpio;
 ...
 * @return	err   : Error code
 */
static void ox01f10_reset_gpio(struct gpio_desc *gpio, int val)
{
