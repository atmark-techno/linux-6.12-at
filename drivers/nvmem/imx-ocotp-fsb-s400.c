...
static int fsb_s400_fuse_write(void *priv, unsigned int offset, void *val, size_t bytes)
{
	...
	mutex_lock(&fuse->lock);
	ret = imx_se_write_fuse(fuse->se_data, index, *buf, lock);
	mutex_unlock(&fuse->lock);
                ...
                struct device_attribute *attr, char *buf)
{
	...
	rc = imx_se_get_events(fuse->se_data, events, &count);
	if (rc) {
                ...
                struct device_attribute *attr, const char *buf, size_t len)
{
	...
	return imx_se_forward_lifecycle(fuse->se_data, LIFECYCLE_OPEN);
}
