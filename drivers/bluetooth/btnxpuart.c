...
static int nxp_serdev_probe(struct serdev_device *serdev)
{
	...
	err = device_property_read_string(&serdev->dev, "btnxpuart,required-netdev",
					  &netdev_name);
	if (!err) {
		/* btnxpuart drivers init is external, wait for
		 * network interface to come up before setting up
		 */
