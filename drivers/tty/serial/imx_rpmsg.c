...
static void imx_rpmsg_uart_tx_work(struct work_struct *ws)
{
	...
	while (!kfifo_is_empty(&tport->xmit_fifo) && !uart_tx_stopped(port) &&
	       !rport->force_end_work) {

		len = kfifo_out_linear_ptr(&tport->xmit_fifo, &tail, RPMSG_MAX_SIZE);

		/* send a message to our remote processor */
		ret = imx_rpmsg_uart_send_and_wait(rport, &msg, tail, len, true, false);
		if (ret) {
		...
		kfifo_skip_count(&tport->xmit_fifo, len);
	}
...
static void imx_rpmsg_uart_shutdown(struct uart_port *port)
{
	...
	mctrl_gpio_disable_ms_sync(rport->gpios);
...
static void imx_rpmsg_uart_platform_remove(struct platform_device *pdev)
{
	struct imx_rpmsg_port *rport = platform_get_drvdata(pdev);

	dev_info(&pdev->dev, "rpmsg tty driver is removed\n");

	uart_remove_one_port(&imx_rpmsg_uart_driver, &rport->port);
}
