...
)
{
    ...
    pDLCI->sMP_DLCI.bNegotiated         = FALSE;
...
)
{
    ...
    pMux->dwMpRevision = LIMIT(dwMpRevision, (DWORD)MP_PROTOCOL_VERSION_MIN, (DWORD)MP_PROTOCOL_VERSION_MAX);
...
)
{
  ...
  pMux->dwMpWindowSize = LIMIT(dwWindowSize, (DWORD)1, (DWORD)7);
  pMux->dwMpFrameSize = LIMIT(dwFrameSize, (DWORD)DEF_MAXFRAMESIZE, (DWORD)MAXFRAMESIZE);
...
)
{
    ...
    if (DLCI_GetProp(pMux, dwDLCI)) {
        ...
        if (pMux->ucInitiator) {
            ...
            {
                // Modules with mux version less than 4 are expected to use a standard mux implementation which require
                // a different mux channel startup procedure. Here 1st send a negotiation request, the connect request
                // will then be triggered by the negotiation confirmation.
                MPFRAME *psFrame = MP_pstFifoGet(&pMux->MP_freelist);
                dwRet = MUX_ERR_GET_MEMORY_FAILED;
                if (psFrame)
                {
                    MP_Write_Negotiation(pMux, psFrame, (UINT8)dwDLCI, FALSE);
